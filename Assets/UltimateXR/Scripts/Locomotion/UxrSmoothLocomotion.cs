...
namespace UltimateXR.Locomotion
{
    ...
    public class UxrSmoothLocomotion : UxrLocomotion
    {
        ...
        [SerializeField]                              private bool _useKeyboardDebug;
        ...
        protected override void UpdateLocomotion()
        {
            if (Avatar)
            {
                ...
                {
                    if (joystickLeft == Vector2.zero)
                    {
                        joystickLeft = new Vector2(Input.GetAxis("Horizontal"), Input.GetAxis("Vertical"));
                    }

                    if (joystickRight == Vector2.zero)
                    {
                        joystickRight = new Vector2(Input.GetAxis("Mouse X"), Input.GetAxis("Mouse Y"));
                    }
                }
